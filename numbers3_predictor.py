...
def train_gpt3numbers_model_with_memory(
    ...
    try:
        ...
        df["本数字"] = df["本数字"].apply()
        sequences = [row for row in df["本数字"] if isinstance(row, list) and len(row) == 4]
    except Exception as e:
    ...
    for seq in sequences:
        for i in range(1, 4):
            context = seq[:i]
            target = seq[i]
            history = sequences[:sequences.index(seq)]  # それ以前の履歴を memory に
    ...
    for epoch in range(epochs):
        ...
        for context, target, hist in data:
            tgt = torch.tensor(context, dtype=torch.long).unsqueeze(1).to(device)  # shape: [seq_len, 1]
            target_tensor = torch.tensor([target], dtype=torch.long).to(device)    # shape: [1]

            memory = build_memory_from_history(hist, encoder, device)  # shape: [mem_len, 1, embed_dim]

            output = decoder(tgt, memory)  # shape: [seq_len, 1, vocab_size]
            last_output = output[-1, 0].unsqueeze(0)  # shape: [1, vocab_size]
...
def train_gpt3numbers_model(save_path="gpt3numbers.pth", epochs=50):
    ...
    data = []
    for seq in sequences:
        for i in range(len(seq) - 1):  # ex: [2,5,3,8] → ([2,5,3], 8)
            context = seq[:i+1]
            target = seq[i+1]
            if len(context) >= 1:
    ...
    for epoch in range(epochs):
        ...
        for context, target in data:
            tgt = torch.tensor(context, dtype=torch.long).unsqueeze(1).to(device)  # (seq_len, batch=1)
            memory = torch.zeros_like(tgt, dtype=torch.float32).to(device)         # ダミーメモリ
            target_tensor = torch.tensor([target], dtype=torch.long).to(device)

            output = model(tgt, memory)[-1].unsqueeze(0)  # 最後の出力トークン
...
def calculate_number_cycle_score(historical_numbers):
    ...
    入力は DataFrame（'本数字' 列に3桁リスト）でも、リスト/ndarray の列挙でもOK。
    """
    seq = []
    try:
        import pandas as pd  # noqa: F401
        if hasattr(historical_numbers, "get") or hasattr(historical_numbers, "__getitem__"):
            if "本数字" in getattr(historical_numbers, "columns", []):
...
def train_meta_model_maml(evaluation_csv_path, data_frame):
    """
    本来的にはMAML等のメタ学習を想定しているようだが、ここでは簡易版:
    - evaluation_result.csv が存在すれば読み込み
    - '信頼度' などの簡易特徴から「当たり（ミニ/ボックス/ストレート）」を分類するRFを学習
    - 無ければ None を返す（呼び出し側で安全に処理される）
    """
    import os
    import pandas as pd
    from sklearn.ensemble import RandomForestClassifier
    ...
    if not os.path.exists(evaluation_csv_path) or os.path.getsize(evaluation_csv_path) == 0:
        print(f"[META] {evaluation_csv_path} が見つからないためメタモデルは作成しません。")
        return None

    try:
        df = pd.read_csv(evaluation_csv_path)
    except Exception as e:
        print(f"[META] 読み込み失敗: {e}")
        return None

    needed = []
    if "信頼度" in df.columns: needed.append("信頼度")
    if "期待収益" in df.columns: needed.append("期待収益")
    if not needed:
        print("[META] 学習に使える数値特徴が不足しています。")
        return None
    if "等級" not in df.columns:
        print("[META] '等級' 列が無いためメタモデルを作成しません。")
        return None

    y = df["等級"].isin(["ミニ", "ボックス", "ストレート"]).astype(int).values
    X = df[needed].fillna(0.0).values

    if len(df) < 10 or y.sum() == 0:
        print("[META] 学習データが少なすぎるか、全てはずれのためスキップ。")
        return None

    clf = RandomForestClassifier(n_estimators=200, random_state=42)
    clf.fit(X, y)
    print("[META] 簡易メタモデルを学習しました（RandomForest）。")
    ...
    class _MetaWrapper:
        ...
        def predict(self, feat_array):
            import numpy as np
            a = np.array(feat_array, dtype=float)
            if a.ndim == 1:
                a = a.reshape(1, -1)
            try:
                ...
                return self.model.predict(fallback)
    return _MetaWrapper(clf)
...
def parse_number_string(value):
    """
    入力が str でも list/ndarray でも安全に [int, int, int] を返す。
    許容フォーマット:
      - "1,2,3" / "1 2 3" / "[1, 2, 3]"
    ...
    if s == "":
        return []
    s = s.strip("[](){}")
    # 数字1桁ずつを拾う（Numbers3前提）
    tokens = re.findall(r"\d", s)
    return [int(t) for t in tokens]


# === Optuna params integration (Numbers3) ===
def _jaccard_set(a, b):
    A, B = set(map(int, a)), set(map(int, b))
    u = len(A | B)
    return (len(A & B) / u) if u else 0.0

def enforce_final_diversity_with_tuples(preds, max_sim=0.6):
    """
    preds: list of (numbers, confidence) or (numbers, confidence, origin)
    Returns diversified list preserving original tuple shapes.
    """
    out = []
    for item in preds:
        numbers = item[0]
        ok = True
        for keep in out:
            if _jaccard_set(numbers, keep[0]) > max_sim:
                ok = False; break
        if ok:
            out.append(item)
    return out

def load_optuna_params(json_path="optuna_results/best_params.json", default_topk=5, default_max_sim=0.6):
    try:
        with open(json_path, "r", encoding="utf-8") as f:
            d = json.load(f)
        return int(d.get("topk", default_topk)), float(d.get("max_sim", default_max_sim))
    except Exception:
        return default_topk, default_max_sim

def apply_optuna_selection(all_predictions):
    """
    Sort by confidence desc, then enforce diversity (Jaccard <= max_sim), then cut to topk.
    """
    topk, max_sim = load_optuna_params()
    try:
        preds_sorted = sorted(all_predictions, key=lambda x: float(x[1]), reverse=True)
    except Exception:
        preds_sorted = all_predictions[:]
    diversified = enforce_final_diversity_with_tuples(preds_sorted, max_sim=max_sim)
...
def add_random_diversity(predictions):
    # 乱択を軽く混ぜて、候補を1つ追加するヘルパー。
    pool = list(range(10))
    random.shuffle(pool)
    base = pool[:3]
    # 既存の先頭候補の先頭数字を混ぜて少しだけ相関を残す
...
def main_with_improved_predictions():
    ...
    # === Optuna tuned selection (topk & diversity) ===
    all_predictions = apply_optuna_selection(all_predictions)
    print("[INFO] Optunaのbest_paramsを適用: 上位候補を選抜 & 多様性制約を適用しました")
# === 検証・保存・評価 ===
...
def enforce_grade_structure(predictions):
    ...
    入力: [(numbers, conf)] or [(numbers, conf, origin)]
    出力: [(numbers(list[int]), conf(float), origin(str))]
    """
    seen = set()
    normalized = []
...
# ==== ▼▼ 最新の抽選日（次回抽選日）を自動判定する追加コード ▼▼ ====
# 既存の get_latest_drawing_dates() を利用して公式サイトから抽せん日一覧を取得し、
# 直近の実施日から「次回の平日抽せん日」を計算します。
# 取得が失敗した場合は、手元CSVや今日の日付から次の平日を推定します。

import asyncio
from datetime import datetime, timedelta

def _to_date(s):
    # "YYYY-MM-DD" / "YYYY/MM/DD" / "YYYY年MM月DD日" 等をゆるく受ける
    if s is None:
        return None
...
async def resolve_target_draw_date(today=None, csv_path="numbers3.csv"):
    """
    1) 公式サイトから抽せん日一覧を取得（get_latest_drawing_dates）
    2) 取得できたら直近の実施日（<= today）を見つけて次の平日を返す
    3) 失敗時は CSV の最新抽せん日から推定
    4) それも無理なら today→次の平日
    戻り値: 'YYYY-MM-DD' 文字列
    """
    if today is None:
        today = datetime.now().date()
...
def resolve_target_draw_date_sync(today=None, csv_path="numbers3.csv"):
    """同期API：外部から簡単に呼べるラッパー"""
    return asyncio.run(resolve_target_draw_date(today=today, csv_path=csv_path))

if __name__ == "__main__":
    # テスト出力（学習や予測は実行せず、日付解決のみ）
    print("[INFO] 次回抽せん日(推定):", resolve_target_draw_date_sync())
# ==== ▲▲ 追加コードここまで ▲▲ ====
